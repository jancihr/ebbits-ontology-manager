...
import eu.om.generator.ServiceGenerator;
...
public class OMRepository {
	...
	public void reloadPrefixMap(){
		System.out.println("RE-INITIALIZING PREFIX MAP:");

		RepositoryConnection conn = this.getConnection();
		try{
			OMRepository.prefixMap.clear();
			System.out.println("got connection");
			RepositoryResult<org.openrdf.model.Namespace> ns = conn.getNamespaces();
			System.out.println("got namespaces");
			while(ns.hasNext()){
				org.openrdf.model.Namespace n = ns.next();
				System.out.println("adding ["+n.getPrefix()+":"+n.getName()+"]");
				OMRepository.prefixMap.put(n.getPrefix(), n.getName());
			}
		}
		catch(Exception e){
			e.printStackTrace();
	...
	public String getDirectInstanceType(String instanceURI){
		...
		try{
			...
			if(result.hasNext()) return result.next().getObject().toString();
		}
		catch(Exception e){
			e.printStackTrace();
		}
	...
	/**
	 * Retrieves graph of all values of all instance properties including inferred, but does not return the instance rdf:type;
	 * @param instanceURI Instance URI.
	...
	/**
	 * Retrieves graph of all values of direct instance properties including inverse properties;
	 * @param instanceURI Instance URI.
	 * @return Property Instance plain property graph;
	 */
	...
	public String value(String var, BindingSet b){
	 ...
	 * @param serviceType name of ontology class of service type in prefixed form (ebbits:BasicDataService) 
	 * @param entityType name of ontology instance of entity to which the service reffers - in prefixed form (ebbits:cow)
	...
	public void query(String query){
		...
		RepositoryConnection conn = getConnection();
		try{
			TupleQueryResult result = sparql(conn, query);
			if(result != null){
				while(result.hasNext()){
					BindingSet b = result.next();
	...
	public String list() {
		...
		try{
			RepositoryResult<Statement> result = 
				conn.getStatements(null, Rdf.rdfType, null, true);
			while(result.hasNext()){
				Statement st = result.next();
				out += "[";
    ...
    public String sparqlXML(String queryString, boolean addPrefixes) throws Exception {
        String xml = "<sparql xmlns=\"http://www.w3.org/2005/sparql-results#\">";
        RepositoryConnection conn = getConnection();
        try{
                String q = "";
                if(addPrefixes){
                        q += Namespace.queryPrefixes();
                }
                ...
                if(result != null){
                        while(result.hasNext()){
                                ...
                                xml += "<result>";
                                for(String var : vars)  { 
                                        xml += "<binding name=\""+var+"\">"+b.getValue(var).stringValue()+"</binding>";
                                }
                                xml += "</result>";
                        }
                }
